...
test("basic error; 0 context", async () => {
  ...
  expect(frames).toEqual(
    JSON.parse(fs.readFileSync(resolve(__dirname, "../../fixtures/bundle.json")).toString("utf8")),
  );
});
...
test("default context (3)", async () => {
  ...
  expect(frames).toEqual(
    JSON.parse(fs.readFileSync(resolve(__dirname, "../../fixtures/bundle-default.json")).toString("utf8")),
  );
});
